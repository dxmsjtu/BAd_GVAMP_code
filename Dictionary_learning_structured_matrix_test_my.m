% Code for paper:"Bilinear Adaptive Generalized Adaptive Vector Approximate Message Passing", IEEE Access, 2018. 
% Test code for Structured dictionary learning Code written by Xiangming Meng and Jiang Zhu Email: mengxm11@gmail.com, jiangzhu16@zju.edu.cn
% 2018, Sep. 27
clear;close all; 
n = 64;          % signal dimension
rate = 1;
NumBits = 3;  % quantization bits
prior_pi = 0.2;prior_mean = 0;prior_var = 1;Afro2 = n;SNR = 40;
global  dampFac T tol 
dampFac = 1;tol = 1e-10;T = 100;  % maximum number of iterations
m = ceil(rate*n);
% L = ceil(5*n*log(n));
L = 10;  tau = zeros(m,1);
Q = n;
...
dMSEb_all = zeros(MC,T);dMSEA_all = zeros(MC,T);dMSEx_all = zeros(MC,T);

dMSEb_oral= zeros(MC,T);dMSE_c_oracle_all= zeros(MC,T);dMSE_b_oracle_all= zeros(MC,T);
